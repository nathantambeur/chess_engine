...
PrincipalVariation Search::pv(const Board& board,const TimeInfo::Optional& timeInfo){
    m_board = board;
    int current_depth = 0;
    int max_depth = 7; //go only up to ply 6 for normal times and passing the tests   
    int score = 0;
    bool checked = false;
    ...
    if (timeInfo.has_value()){
        max_depth = 8; //if official time is kept try to go upto ply 7 anything more will result in slow playtimes and wasting time   
        ...
        if (total_time_left < std::chrono::seconds(20)){
            //if less then 4 minute left then only search 4 plys for instant but good performance, having super short time left makes it sometimes return very bad results 
            time_per_turn = std::chrono::minutes(100);
            bonus_time_next_turn= std :: chrono ::minutes(0);
            max_depth = 5;
        ...
        else if (total_time_left < std::chrono::minutes(5)){
            //if less then 4 minute left then only search 6 plys for fast but good performance, having super short time left makes it sometimes return very bad results 
            time_per_turn = std::chrono::minutes(100);
            bonus_time_next_turn= std :: chrono ::minutes(0);
            max_depth = 7;
...
//evalute the current m_board state with PESTO tables tables and adapted from https://www.chessprogramming.org/PeSTO%27s_Evaluation_Function (original table: https://rofchade.nl/?p=307)
